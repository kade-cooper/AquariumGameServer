...
function verifyJwt(token) {
  if (!CLIENT_SECRET || !token) return null;
  ...
  try {
    ...
    const expected = crypto.createHmac("sha256", CLIENT_SECRET).update(h + "." + p).digest("base64url");
    const sig = Buffer.from(s, "base64url");
    if (sig.length !== expected.length || !crypto.timingSafeEqual(sig, Buffer.from(expected))) return null;
...
function ensureLoop(channelId) {
  const ch = getChannel(channelId);
  if (!ch.loopTimer) {
    ch.loopTimer = setInterval(() => tick(channelId), 500);
  }
}
